...
class RAGEngine {
    constructor() {
        this.vectorsDir = path.join(process.cwd(), 'data', 'vectors');
    ...
    async buildIndex(documents, geminiClient) {
        ...
        // Peta teks hash -> vector dari index lama, sebagai cache
        const cache = new Map();
        ...
        for (let i = 0; i < documents.length; i++) {
            ...
            if (!vector) {
                ...
                try {
                    ...
                    if (vector) {
                        // Delay sedikit agar tidak terkena rate limit 429
    ...
    loadIndex() {
        try {
            ...
            if (fs.existsSync(indexPath)) {
                ...
                console.log(`⚠️ ERROR: File index embeddings tidak ditemukan di: ${indexPath}`);
            }
        } catch (error) {
    ...
    saveIndex() {
        try {
            if (process.env.VERCEL) {
                console.log('Vercel: skip saveIndex (read-only filesystem)');
                return true;
            }
            const indexPath = path.join(this.vectorsDir, 'embeddings.json');
